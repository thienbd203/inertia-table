...
The package follows semantic versioning from `v1.0`; the resource/API contract uses
`schemaVersion: 2`. Treat documented resource fields, URL state, queued
snapshots, Saved Views, exports and extension hooks as compatibility-sensitive.
...
CI runs PHP compatibility jobs across the supported PHP, Laravel, Testbench and
Inertia combinations, including SQLite plus dedicated MySQL/PostgreSQL jobs.
JavaScript checks run separately on the supported Node versions. Run focused
checks locally; rely on the workflows for the full matrix.
...
- **`useActions(table, options, callbacks)`** (`resources/js/useActions.ts`) owns row/bulk selection (keyed via an explicit `rowKey`, then the server-serialized `item._table.key`, then `item.id`), current-page Shift-click ranges, action execution (`router.visit` for endpoint-backed actions, or the `onCustomAction` callback for frontend-owned actions), and confirmation flow. The header checkbox immediately selects all search/filter matches across pagination; unchecked rows become exclusions in the selection descriptor. Selection is intentionally not part of URL state and is cleared when active search or filters change.
